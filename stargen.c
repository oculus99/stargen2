...
long double	cloud_eccentricity=0.2;
long double nearestk=0.3;
long double farthestk=50.0;
long double diskradiusk=200;
long double alphaa=5.0; // ALPHA 5?
...
planets mercury1={ 1,0.387, 0.206, 0, 0, 0
 , 0, 0, ZEROES,0,NULL, &jupiter1};
...
// --- ASSUMED EXTERNAL DEFINITIONS (PLACEHOLDERS) ---
// You MUST ensure these match your actual stargen definitions.
// Typically found in a header file like "stargen.h" or similar.
// #include "YOUR_STARGEN_HEADER.h"

#define SOLAR_MASS_IN_EARTH_MASSES 333000.0L
#define EARTH_MASS 1.0L
#define ASTEROID_MASS_THRESHOLD 1.0e-6L // This is 1.0e-6 Earth masses



// Function to properly allocate and initialize a new planet struct on the heap
// This function should be used *where you generate new planets*.
planet_pointer create_new_planet(int p_no, long double p_a, long double p_mass, int is_gas_giant, long double p_orb_period, sun* star_ptr, planet_type p_type) {
    planet_pointer new_planet = (planet_pointer)malloc(sizeof(planets));
    if (new_planet == NULL) {
        fprintf(stderr, "ERROR: Failed to allocate memory for a new planet.\n");
        return NULL;
    }

    // Initialize all fields. Crucial to set next_planet to NULL initially.
    new_planet->planet_no = p_no;
    new_planet->a = p_a;
    new_planet->mass = p_mass;
    new_planet->gas_giant = is_gas_giant;
    new_planet->orb_period = p_orb_period;
    new_planet->sun = star_ptr;
    new_planet->next_planet = NULL; // IMPORTANT: Initialize to NULL
    new_planet->type = p_type;     // Set the new planet_type field

    // Initialize other fields of 'planets_record' to default/safe values
    // This prevents garbage data if you don't explicitly set them later.
    new_planet->e = 0.0L;
    new_planet->axial_tilt = 0.0L;
    new_planet->dust_mass = 0.0L;
    new_planet->gas_mass = 0.0L;
    new_planet->moon_a = 0.0L;
    new_planet->moon_e = 0.0L;
    new_planet->core_radius = 0.0L;
    new_planet->radius = 0.0L;
    new_planet->orbit_zone = 0;
    new_planet->density = 0.0L;
    new_planet->day = 0.0L;
    new_planet->resonant_period = 0;
    new_planet->esc_velocity = 0.0L;
    new_planet->surf_accel = 0.0L;
    new_planet->surf_grav = 0.0L;
    new_planet->rms_velocity = 0.0L;
    new_planet->molec_weight = 0.0L;
    new_planet->volatile_gas_inventory = 0.0L;
    new_planet->surf_pressure = 0.0L;
    new_planet->greenhouse_effect = 0;
    new_planet->boil_point = 0.0L;
    new_planet->albedo = 0.0L;
    new_planet->exospheric_temp = 0.0L;
    new_planet->estimated_temp = 0.0L;
    new_planet->estimated_terr_temp = 0.0L;
    new_planet->surf_temp = 0.0L;
    new_planet->greenhs_rise = 0.0L;
    new_planet->high_temp = 0.0L;
    new_planet->low_temp = 0.0L;
    new_planet->max_temp = 0.0L;
    new_planet->min_temp = 0.0L;
    new_planet->hydrosphere = 0.0L;
    new_planet->cloud_cover = 0.0L;
    new_planet->ice_cover = 0.0L;
    new_planet->gases = 0;
    // new_planet->atmosphere = NULL; // If 'atmosphere' is a pointer, uncomment and initialize
    new_planet->minor_moons = 0;
...
int compare_planets_by_mass(const void* a, const void* b) {
    ...
    planet_pointer p2 = *(planet_pointer*)b;

    if (p1 == NULL && p2 == NULL) return 0;
    if (p1 == NULL) return 1; // NULLs go to the end
    if (p2 == NULL) return -1; // NULLs go to the end

    // Sort in descending order of mass (more massive first)
    // p1->mass < p2->mass means p1 is smaller, so p2 should come first (return -1 for qsort)
    if (p1->mass < p2->mass) return 1;
    if (p1->mass > p2->mass) return -1;

    // If masses are equal, sort by semi-major axis in descending order for deterministic results
    if (p1->a < p2->a) return 1;
    if (p1->a > p2->a) return -1;

    return 0; // Planets are effectively identical for sorting purposes
}

// Helper function to calculate Hill Radius
long double calculate_hill_radius(planet_pointer p, sun* star) {
    if (p == NULL || star == NULL || star->mass <= LDBL_EPSILON || p->mass <= LDBL_EPSILON || p->a <= LDBL_EPSILON) {
        if (p != NULL) {
            fprintf(stderr, "WARNING: Cannot calculate Hill radius for Planet %d due to invalid data (mass: %.2e, a: %.2Lf, star_mass: %.2e).\n",
                    p->planet_no, p->mass, p->a, (star != NULL ? star->mass : 0.0L));
        } else {
             fprintf(stderr, "WARNING: Cannot calculate Hill radius due to NULL planet pointer.\n");
        }
        return 0.0L; // Return 0.0 if any critical input is invalid
    }
    return p->a * powl(p->mass / (3.0L * star->mass), 1.0L/3.0L);
}

// Function to convert linked list to array for sorting
planet_pointer* linked_list_to_array(planet_pointer head, int* count) {
    int num_planets = 0;
    ...
    // CRITICAL FIX: Allocate space for pointers, not structs
    planet_pointer* planet_array = (planet_pointer*)malloc(num_planets * sizeof(planet_pointer));
    if (planet_array == NULL) {
        fprintf(stderr, "ERROR: Memory allocation failed for planet_array in linked_list_to_array.\n");
        *count = 0;
        return NULL;
    }
...
// Function to free the array (not the planet nodes themselves)
void free_planet_array(planet_pointer* arr) {
    if (arr) {
        free(arr);
    }
}
